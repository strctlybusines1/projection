...
import config
config.DK_AVG_BLEND_WEIGHT = 0.9
...
summary = dict(
    run_id=1, label='Triple-mix + 90/10 blend',
    params=dict(dk_avg_blend=0.9, primary_stack_boost=0.2,
                goalie_corr_boost=0.1, use_diverse=False),
    n_slates=len(rdf),
...
with open('/Users/brendanhorlbeck/Desktop/Code/projection/backtest_results/run_1.json', 'w') as f:
    json.dump(summary, f, indent=2)
print(f"Run 1: {len(rdf)} slates, edge={rdf['m3s_edge'].mean():+.1f}, {elapsed:.0f}s")
