...
# Use our own projections when available (my_proj from dk_salaries), fall back to FC Proj from own.csv
df['proj'] = df['my_proj'].fillna(df['FC Proj']) if 'my_proj' in df.columns else df['FC Proj']
df['value_score'] = df['proj'] / (df['Salary'] / 1000)  # proj per 1k salary
...
feature_cols = [
    ...
    # Projection & value
    'proj', 'value_score', 'value_percentile', 'value_rank_position',
    # Line & PP
...
model_df = df[feature_cols + ['date', 'Ownership', 'Pos', 'Player', 'name_norm',
                               'is_se', 'contest$', 'FPs', 'proj', 'Team']].copy()
...
proj_vals = combined_df['proj'].iloc[:, 0].values if isinstance(combined_df['proj'], pd.DataFrame) else combined_df['proj'].values
own_pred = combined_df['own_pred'].values
leverage = proj_vals / (own_pred + 1)
...
combined_df['is_high_leverage'] = (
    (combined_df['proj'] > combined_df['proj'].quantile(0.75)) &
...
combined_df['is_contrarian'] = (
    ...
    (combined_df['proj'] < combined_df['proj'].quantile(0.5))
).astype(int)
...
if len(hl) > 0:
    ...
    print(f"  Avg Projection: {hl['proj'].mean():.2f}")
...
if len(cn) > 0:
    ...
    print(f"  Avg Projection: {cn['proj'].mean():.2f}")
...
if len(ch) > 0:
    ...
    print(f"  Avg Projection: {ch['proj'].mean():.2f}")
...
available_cols = ['date', 'Player', 'Pos', 'Team', 'proj', 'own_pred',
