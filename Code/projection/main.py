...
from lines import LinesScraper, StackBuilder, print_team_lines
...
def merge_projections_with_salaries(projections: pd.DataFrame,
    ...
    # Clean names for matching (transliterate unicode accents, lowercase, strip)
    proj['name_clean'] = proj['name'].apply(clean_name)
    sal['name_clean'] = sal['dk_name'].apply(clean_name)
...
def main():
    ...
    # HMM regime edge signals
    parser.add_argument('--hmm', action='store_true',
                        help='Apply HMM regime signals (opponent matchup + downgrade bounce)')
    parser.add_argument('--hmm-report', action='store_true',
                        help='Print full HMM edge report for slate')
    ...
    # Component selection (swappable implementations)
    parser.add_argument('--optimizer', type=str, default='greedy',
                        choices=['greedy', 'ilp'],
                        help='Optimizer implementation: greedy (default) or ilp (integer linear programming)')
    parser.add_argument('--sim-engine', type=str, default='basic',
                        choices=['basic', 'correlated'],
                        help='Simulator engine: basic (default) or correlated (zero-inflated lognormal MC)')
    ...
    # Apply HMM regime edge signals if enabled
    if args.hmm and len(skaters_merged) > 0:
        try:
            from hmm_edge import HMMEdgeModel
            print("\nApplying HMM regime signals...")
            hmm_model = HMMEdgeModel()
            hmm_model.fit()

            # Get edges for all players on slate
            hmm_edges = hmm_model.get_slate_edges(dk_salaries)

            # Merge HMM boosts into skaters
            boosted_count = 0
            stacked_count = 0
            for idx, row in skaters_merged.iterrows():
                match = hmm_edges[hmm_edges['Name'] == row['name']]
                if match.empty:
                    continue
                boost = match.iloc[0]['HMM_Boost']
                if boost != 1.0:
                    skaters_merged.at[idx, 'projected_fpts'] *= boost
                    boosted_count += 1
                    if match.iloc[0].get('Downgrade', False):
                        stacked_count += 1

                # Store signal metadata for ownership model / reports
                skaters_merged.at[idx, 'hmm_boost'] = boost
                skaters_merged.at[idx, 'hmm_state'] = match.iloc[0].get('HMM_State', '')
                skaters_merged.at[idx, 'hmm_signal'] = match.iloc[0].get('Reasons', '')

            # Recalculate value and edge after HMM boost
            ...
            print(f"  HMM boosts applied to {boosted_count} skaters "
                  f"({stacked_count} downgrades)")

            # Print full report if requested
            if args.hmm_report:
                hmm_model.print_report(hmm_edges)

        except ImportError:
            print("  Warning: hmm_edge module not found — install hmmlearn")
        except Exception as e:
            print(f"  Warning: HMM edge signals failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    if not args.no_stacks or args.stacks:
        ...
        if confirmed:
            _fm = fuzzy_match  # already imported from utils
    ...
    if len(skaters_merged) > 0 and len(goalies_merged) > 0:
        ...
        if args.single_entry:
            ...
            NHLLineupOptimizer = get_optimizer(args.optimizer)
