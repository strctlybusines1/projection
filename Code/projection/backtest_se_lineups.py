...
def get_actuals_from_db(date_str: str) -> pd.DataFrame:
    ...
    if not actuals.empty:
        # Normalize team abbreviations: DK uses NJ/LA/SJ/TB, game logs use NJD/LAK/SJS/TBL
        TEAM_NORM = {'nj': 'njd', 'la': 'lak', 'sj': 'sjs', 'tb': 'tbl'}
        actuals['_team_norm'] = actuals['team'].str.lower().str.strip().map(lambda t: TEAM_NORM.get(t, t))
        actuals['_key'] = actuals['name'].str.lower().str.strip() + '_' + actuals['_team_norm']
    return actuals
...
def score_lineup_actual(lineup: pd.DataFrame, actuals: pd.DataFrame) -> float:
    ...
    lineup = lineup.copy()
    TEAM_NORM = {'nj': 'njd', 'la': 'lak', 'sj': 'sjs', 'tb': 'tbl'}
    lineup['_key'] = lineup['name'].str.lower().str.strip() + '_' + lineup['team'].str.lower().str.strip().map(lambda t: TEAM_NORM.get(t, t))
...
def run_se_backtest_date(date_str: str, n_candidates: int = 40, verbose: bool = True):
    ...
    if verbose:
        ...
        # Coverage check (with team normalization)
        TEAM_NORM_COV = {'nj': 'njd', 'la': 'lak', 'sj': 'sjs', 'tb': 'tbl'}
        pool_key = pool['name'].str.lower().str.strip() + '_' + pool['team'].str.lower().str.strip().map(lambda t: TEAM_NORM_COV.get(t, t))
