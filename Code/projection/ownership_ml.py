...
class RuleBasedOwnershipModel:
    ...
    def predict(self, df):
        ...
        2. Value score: proj / (salary/1000)
        ...
        # Value score
        df['value_score'] = df['proj'] / (df['salary'] / 1000.0 + 1)
        ...
        proj_median = df.groupby(['slate_date', 'position'])['proj'].transform('median')
        proj_ratio = (df['proj'] / (proj_median + 0.1)).clip(0.3, 3.0)
...
def engineer_features(df, historical_data=None):
    ...
    # Projection vs DK average alignment (how much we deviate from public baseline)
    df['proj_alignment'] = df['proj'] / (df['dk_avg_fpts'].fillna(df['proj']) + 0.01)
    ...
    df['proj_rank'] = df.groupby(['slate_date', 'position'])['proj'].rank(ascending=False)
    df['proj_percentile'] = df.groupby('slate_date')['proj'].rank(pct=True)
    ...
    if historical_data is not None:
        ...
        df['historical_own_rate'] = df['historical_own_rate'].fillna(df['proj'].median() / 20)
        df['historical_own_std'] = df['historical_own_std'].fillna(2.0)
    else:
        df['historical_own_rate'] = df['proj'] / 20  # Fallback: projection-based
        df['historical_own_std'] = 2.0

    # === DERIVED METRICS ===
    # Salary efficiency
    df['salary_efficiency'] = df['proj'] / (df['salary'] / 1000.0 + 1)
...
class XGBoostOwnershipModel:
    ...
    def get_feature_set(self):
        ...
        return [
            ...
            # Projection
            'proj_rank', 'proj_percentile', 'avg_fpts_rank',
...
class GameTheoryAdjustments:
    ...
    def compute_leverage_score(df):
        ...
        """
        df['leverage_score'] = df['proj'] / (df['predicted_ownership'] + 1.0)
        return df
    ...
    def compute_contrarian_flag(df):
        ...
        """
        df['proj_median_pos'] = df.groupby(['slate_date', 'position'])['proj'].transform('median')
        df['contrarian_flag'] = (
            (df['predicted_ownership'] < 5.0) &
            (df['proj'] > df['proj_median_pos'])
        ).astype(int)
        return df
...
def main():
    ...
    # Use our own projections (my_proj) when available, fall back to fc_proj from DB
    if 'my_proj' in dk_salaries.columns and 'fc_proj' in dk_salaries.columns:
        dk_salaries['proj'] = dk_salaries['my_proj'].fillna(dk_salaries['fc_proj'])
    elif 'fc_proj' in dk_salaries.columns:
        dk_salaries.rename(columns={'fc_proj': 'proj'}, inplace=True)
    dk_salaries.drop(columns=['my_proj'], errors='ignore', inplace=True)
    ...
    X = train_data.drop(['own_pred', 'slate_date', 'player_name', 'team', 'position', 'opponent',
                         ...
                         'dk_avg_fpts', 'dk_stdv', 'dk_ceiling', 'proj', 'ownership_pct',
    ...
    if len(jan1_slate) > 0:
        sample = jan1_slate.nlargest(10, 'predicted_ownership')[
            ['player_name', 'position', 'salary', 'proj', 'predicted_ownership',
    ...
    output_cols = [
        'slate_date', 'player_name', 'position', 'team', 'salary', 'proj',
