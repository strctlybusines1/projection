...
try:
    from pulp import LpProblem, LpMaximize, LpVariable, LpBinary, lpSum, PULP_CBC_CMD, value, LpStatus
    HAS_PULP = True
except ImportError:
    HAS_PULP = False

# Ledoit-Wolf correlation artifact path
LW_CORR_PATH = Path(__file__).parent / 'data' / 'lw_correlations.json'
...
# Stacking config (data: 2-team lineups win 1.42%, 3-team 1.31%)
# Stack sizes vary per iteration for diversity:
#   60% of iterations: 4+3 (standard)
#   25% of iterations: 5+3 (heavy primary — 2-team build, higher ceiling)
#   15% of iterations: 3+3 (lighter primary — more flex for value fills)
STACK_CONFIGS = [
    (4, 3, 0.60),  # (primary, secondary, probability)
    (5, 3, 0.25),
    (3, 3, 0.15),
]
PRIMARY_STACK_SIZE = 4   # default (used when not randomizing)
SECONDARY_STACK_SIZE = 3
MAX_FROM_TEAM = 6  # Allow up to 6 from one team
MIN_TEAMS = 2      # Minimum unique teams — relaxed from 3 to allow 5+3 builds
...
def load_transformer_backtest_results(date_str: str = None) -> pd.DataFrame:
    """
    Load Transformer v1 backtest results.
    If date_str provided, filter to that date only.
    """
    path = Path(__file__).parent / 'data' / 'transformer_v1_backtest_results.csv'
    if not path.exists():
        print("Warning: transformer_v1_backtest_results.csv not found")
        return pd.DataFrame()
...
def load_ensemble_projections(date_str: str, mdn_weight: float = 0.73,
                               trans_weight: float = 0.27) -> pd.DataFrame:
    """
    Load and blend MDN v3 + Transformer v1 projections (ensemble).

    After wiring PBP-derived advanced stats into MDN v3:
    - MDN v3 alone: MAE 4.036 (was 4.091 before advanced stats)
    - Position-aware optimization: C=62%, D=93%, L=68%, R=78% MDN
    - Overall optimal weight: 73% MDN / 27% Transformer
    - Stacking meta-learner added no lift over weighted average
    """
    mdn = load_mdn_backtest_results(date_str)
    trans = load_transformer_backtest_results(date_str)

    if mdn.empty and trans.empty:
        return pd.DataFrame()

    # If only one model available, use it alone
    if trans.empty:
        print("  Transformer results not available, using MDN v3 only")
        return mdn
    if mdn.empty:
        print("  MDN results not available, using Transformer v1 only")
        trans = trans.rename(columns={'player_name': 'player_name'})
        return trans

    # Build match keys for merging
    mdn['_match_key'] = mdn['player_name'].apply(
        lambda n: normalize_name_for_key(n) if pd.notna(n) else ''
    )
    trans['_match_key'] = trans['player_name'].apply(
        lambda n: normalize_name_for_key(n) if pd.notna(n) else ''
    )

    # Merge on match key + date
    merged = mdn.merge(
        trans[['game_date', '_match_key', 'predicted_fpts', 'std_fpts']],
        on=['game_date', '_match_key'],
        how='left',
        suffixes=('_mdn', '_trans')
    )

    # Blend predictions where both available
    has_both = merged['predicted_fpts_trans'].notna()
    merged['predicted_fpts'] = merged['predicted_fpts_mdn']  # default to MDN
    merged.loc[has_both, 'predicted_fpts'] = (
        mdn_weight * merged.loc[has_both, 'predicted_fpts_mdn'] +
        trans_weight * merged.loc[has_both, 'predicted_fpts_trans']
    )

    # Blend std (conservative: use max of both for uncertainty)
    merged['std_fpts'] = merged['std_fpts_mdn']
    if 'std_fpts_trans' in merged.columns:
        merged.loc[has_both, 'std_fpts'] = np.sqrt(
            mdn_weight * merged.loc[has_both, 'std_fpts_mdn']**2 +
            trans_weight * merged.loc[has_both, 'std_fpts_trans']**2
        )

    match_rate = has_both.sum() / len(merged) * 100
    print(f"  Ensemble blend: {has_both.sum()}/{len(merged)} matched ({match_rate:.1f}%), "
          f"weights: MDN={mdn_weight:.0%} + Trans={trans_weight:.0%}")
...
def load_dk_salaries_for_date(date_str: str) -> pd.DataFrame:
    ...
    df = pd.read_sql(f"""
        SELECT player_name as name, team, position, salary,
               dk_avg_fpts, dk_ceiling, dk_stdv, opponent,
...
# ==============================================================================
# LEDOIT-WOLF CORRELATED SAMPLING HELPERS
# ==============================================================================

def load_lw_correlations() -> Optional[Dict]:
    """Load the Ledoit-Wolf pairwise correlation matrix from artifact."""
    if not LW_CORR_PATH.exists():
        return None
    with open(LW_CORR_PATH, 'r') as f:
        return json.load(f)


def map_players_to_role_slots(pool: pd.DataFrame) -> Dict[int, str]:
    """
    Map each player in the pool to a Ledoit-Wolf role slot (C1/C2/W1-W4/D1-D3).

    Uses norm_pos + start_line from dk_salaries. For wingers on the same line,
    breaks ties by salary (higher salary = higher slot).

    Returns:
        Dict mapping pool DataFrame index → role slot string (or None if unmapped)
    """
    slot_map = {}

    # Only map skaters with valid start_line
    skaters = pool[
        (pool['norm_pos'].isin(['C', 'W', 'D'])) &
        (pool['start_line'].notna()) &
        (~pool['start_line'].isin(['', 'confirm', 'unlikely']))
    ].copy()

    if skaters.empty:
        return slot_map

    # Convert start_line to int safely
    skaters['_line_num'] = pd.to_numeric(skaters['start_line'], errors='coerce')
    skaters = skaters.dropna(subset=['_line_num'])
    skaters['_line_num'] = skaters['_line_num'].astype(int)

    for team in skaters['team'].unique():
        team_df = skaters[skaters['team'] == team]
        used_slots = set()

        # --- Centers: C on line 1 → C1, line 2 → C2 (one per slot, highest salary wins) ---
        centers = team_df[team_df['norm_pos'] == 'C'].sort_values(
            ['_line_num', 'salary'], ascending=[True, False]
        )
        for _, row in centers.iterrows():
            line = row['_line_num']
            slot = None
            if line == 1 and 'C1' not in used_slots:
                slot = 'C1'
            elif line == 2 and 'C2' not in used_slots:
                slot = 'C2'
            if slot:
                slot_map[row.name] = slot
                used_slots.add(slot)

        # --- Wingers: Line 1 → W1/W2, Line 2 → W3/W4 ---
        # Within each line, rank by salary desc (higher salary = higher slot)
        wingers = team_df[team_df['norm_pos'] == 'W'].sort_values(
            ['_line_num', 'salary'], ascending=[True, False]
        )
        w_line1_count = 0
        w_line2_count = 0
        for _, row in wingers.iterrows():
            line = row['_line_num']
            if line == 1 and w_line1_count < 2:
                w_line1_count += 1
                slot = 'W1' if w_line1_count == 1 else 'W2'
                if slot not in used_slots:
                    slot_map[row.name] = slot
                    used_slots.add(slot)
            elif line == 2 and w_line2_count < 2:
                w_line2_count += 1
                slot = 'W3' if w_line2_count == 1 else 'W4'
                if slot not in used_slots:
                    slot_map[row.name] = slot
                    used_slots.add(slot)

        # --- Defensemen: Line 1 → D1, Line 2 → D2, Line 3 → D3 ---
        defense = team_df[team_df['norm_pos'] == 'D'].sort_values(
            ['_line_num', 'salary'], ascending=[True, False]
        )
        for _, row in defense.iterrows():
            line = row['_line_num']
            slot = None
            if line == 1 and 'D1' not in used_slots:
                slot = 'D1'
            elif line == 2 and 'D2' not in used_slots:
                slot = 'D2'
            elif line == 3 and 'D3' not in used_slots:
                slot = 'D3'
            if slot:
                slot_map[row.name] = slot
                used_slots.add(slot)
...
class MonteCarloLineupBuilder:
    ...
    def __init__(self, player_pool: pd.DataFrame, n_iterations: int = MC_ITERATIONS):
        ...
        # --- Ledoit-Wolf correlated sampling setup ---
        self._lw_data = load_lw_correlations()
        self._player_slots = {}       # pool index → role slot string
        self._team_index_groups = {}   # team → list of pool indices with LW slots
        self._team_cholesky = {}       # team → (indices, L_matrix) precomputed Cholesky

        if self._lw_data is not None:
            pw = self._lw_data.get('pairwise_matrix', {})
            self._player_slots = map_players_to_role_slots(self.pool)

            # Group mapped players by team
            team_groups = defaultdict(list)
            for idx, slot in self._player_slots.items():
                team = self.pool.loc[idx, 'team']
                team_groups[team].append((idx, slot))

            # Precompute Cholesky decompositions per team
            n_correlated = 0
            for team, idx_slot_pairs in team_groups.items():
                if len(idx_slot_pairs) < 2:
                    continue
                indices = [p[0] for p in idx_slot_pairs]
                slots = [p[1] for p in idx_slot_pairs]
                n = len(indices)
                ...
                for a in range(n):
                    for b in range(a + 1, n):
                        r = pw.get(slots[a], {}).get(slots[b], 0.0)
                        corr[a, b] = r
                        corr[b, a] = r

                # Cholesky decomposition (correlation matrix must be PD)
                try:
                    L = np.linalg.cholesky(corr)
                    self._team_cholesky[team] = (indices, L)
                    n_correlated += len(indices)
                except np.linalg.LinAlgError:
                    # Non-PD (shouldn't happen with LW shrinkage), skip team
                    pass

            mapped_total = len(self._player_slots)
            n_teams_corr = len(self._team_cholesky)
            print(f"  LW correlations: {mapped_total} players mapped to role slots, "
                  f"{n_teams_corr} teams with correlated sampling ({n_correlated} players)")

    def _sample_fpts(self) -> pd.Series:
        """
        Sample FPTS with Ledoit-Wolf correlated draws for same-team players.

        For each team with 2+ mapped players:
        1. Draw independent standard normals z ~ N(0,1) for each player
        2. Apply Cholesky(LW_corr) to induce empirical correlation: z_corr = L @ z
        3. Scale: sampled = projected + std * z_corr

        Players without role slots (line 3+, goalies) remain independent.
        """
        ...
        z = np.random.standard_normal(len(projected))

        # Apply team-level LW correlations via precomputed Cholesky
        for team, (indices, L) in self._team_cholesky.items():
            # Get the pool-level integer positions for these indices
            positions = [self.pool.index.get_loc(idx) for idx in indices]
            z_indep = z[positions]
            z_corr = L @ z_indep
            for k, pos in enumerate(positions):
                z[pos] = z_corr[k]

        sampled = np.maximum(projected + std * z, 0)
    ...
    def _build_single_lineup(self, sampled_fpts: pd.Series, leverage_weight: float = 0.3) -> Optional[Dict]:
        ...
        Uses a greedy stacking approach with randomized stack sizes:
        1. Pick stack config (4+3, 5+3, or 3+3) based on weighted random
        2. Pick primary stack team (weighted random from top teams by sampled total)
        3. Pick N best skaters from primary team
        4. Pick secondary stack team
        5. Pick M best skaters from secondary team
        6. Fill remaining spots from other teams
        7. Pick best goalie (not opposing any skater team)
        """
        pool = self.pool.copy()
        ...
        # Randomize stack config this iteration
        config_probs = [c[2] for c in STACK_CONFIGS]
        config_idx = np.random.choice(len(STACK_CONFIGS), p=config_probs)
        primary_size, secondary_size, _ = STACK_CONFIGS[config_idx]
        ...
        # Step 2: Primary stack (variable size)
        ...
        return {
            ...
            'stack_config': f'{primary_size}+{secondary_size}',
        }
    ...
    def _build_single_lineup_ilp(self, sampled_fpts: pd.Series, leverage_weight: float = 0.3) -> Optional[Dict]:
        """
        Build a single lineup from sampled FPTS using ILP (Integer Linear Programming).

        Mathematically optimal replacement for _build_single_lineup():
        - Guarantees salary-cap-optimal solution under stacking constraints
        - No greedy approximation errors (e.g., leaving $1k+ unused salary)
        - Position fills are globally optimal, not order-dependent

        Uses PuLP/CBC solver (~15ms per solve).
        """
        pool = self.pool.copy()
        pool['sampled_fpts'] = sampled_fpts.values
        ...
        # ILP requires no NaN/inf in scores
        pool['selection_score'] = pool['selection_score'].fillna(0).replace([np.inf, -np.inf], 0)
        ...
        # Select primary and secondary teams (weighted random)
        top_teams = team_totals.head(5)
        weights = np.array([0.35, 0.25, 0.20, 0.12, 0.08][:len(top_teams)])
        weights = weights / weights.sum()
        primary_team = np.random.choice(top_teams.index, p=weights)
        ...
        # Build ILP problem
        n = len(pool)
        prob = LpProblem("mc_lineup", LpMaximize)
        x = [LpVariable(f"x_{i}", cat=LpBinary) for i in range(n)]

        # UTIL eligibility (C/W only)
        u = [LpVariable(f"u_{i}", cat=LpBinary) for i in range(n)]

        # Objective: maximize selection score
        scores = pool['selection_score'].values
        prob += lpSum(scores[i] * x[i] for i in range(n))

        # Exactly 9 players
        prob += lpSum(x) == 9

        # Salary cap + floor (use at least 90% of cap for competitive lineups)
        salaries = pool['salary'].values
        prob += lpSum(salaries[i] * x[i] for i in range(n)) <= SALARY_CAP
        prob += lpSum(salaries[i] * x[i] for i in range(n)) >= int(SALARY_CAP * 0.90)

        # Position masks
        is_c = (pool['norm_pos'] == 'C').values
        is_w = (pool['norm_pos'] == 'W').values
        is_d = (pool['norm_pos'] == 'D').values
        is_g = (pool['norm_pos'] == 'G').values

        # Exactly 1 goalie
        prob += lpSum(x[i] for i in range(n) if is_g[i]) == 1

        # UTIL constraints (C/W only)
        for i in range(n):
            prob += u[i] <= x[i]
            if is_d[i] or is_g[i]:
                prob += u[i] == 0
        ...
        for team in pool['team'].unique():
            mask = (pool['team'] == team).values
            prob += lpSum(x[i] for i in range(n) if mask[i]) <= MAX_FROM_TEAM

        # Minimum teams (skaters only)
        skater_teams = pool[~pool['norm_pos'].isin(['G'])]['team'].unique()
        t_vars = {}
        for team in skater_teams:
            t_vars[team] = LpVariable(f"t_{team}", cat=LpBinary)
            team_skater_mask = ((pool['team'] == team) & (pool['norm_pos'] != 'G')).values
            for i in range(n):
        ...
        # Primary stack minimum
        primary_skater_mask = ((pool['team'] == primary_team) & (pool['norm_pos'] != 'G')).values
        prob += lpSum(x[i] for i in range(n) if primary_skater_mask[i]) >= primary_size

        # Secondary stack minimum
        secondary_skater_mask = ((pool['team'] == secondary_team) & (pool['norm_pos'] != 'G')).values
        prob += lpSum(x[i] for i in range(n) if secondary_skater_mask[i]) >= secondary_size

        # Goalie vs opponent exclusion
        for i in range(n):
            if is_g[i]:
                goalie_team = str(pool.iloc[i]['team']).upper()
                goalie_opp = self.opponent_map.get(goalie_team)
                if goalie_opp:
                    opp_skaters = ((pool['team'].str.upper() == goalie_opp) & (pool['norm_pos'] != 'G')).values
                    for j in range(n):
                        if opp_skaters[j]:
                            prob += x[i] + x[j] <= 1
        ...
        util_idx = [i for i in range(n) if value(u[i]) > 0.5]
        ...
        # Assign roster slots
        lineup = []
        util_player = util_idx[0] if util_idx else None
        ...
        total_salary = sum(p['salary'] for p in lineup)
        primary_count = sum(1 for p in lineup if p['team'] == primary_team and p['roster_slot'] != 'G')
        secondary_count = sum(1 for p in lineup if p['team'] == secondary_team and p['roster_slot'] != 'G')
    ...
    def run(self, n_lineups: int = MC_TOP_SELECT) -> Tuple[List[Dict], pd.DataFrame]:
        ...
        # Greedy builder (proven effective in backtests)
        # ILP builder available via _build_single_lineup_ilp() but requires
        # objective function tuning before deployment
        ...
        # Kelly Criterion lineup scoring
        # edge = projected upside over estimated cash line
        # kelly_score = edge² / variance × ownership_leverage
        # This rewards: high projected totals, low variance (consistent), low ownership
        if all_lineups:
            projected_totals = [lu['total_projected'] for lu in all_lineups]
            median_projected = np.median(projected_totals)
            # Estimate cash line as ~median projected (conservative)
            estimated_cash_line = median_projected * 0.95

            for lu in all_lineups:
                edge = max(lu['total_projected'] - estimated_cash_line, 0.1)
                variance = sum(p.get('std_fpts', 5.5) ** 2 for p in lu['players'])
                avg_own = lu['avg_ownership']
                own_leverage = 1.0 / np.sqrt(max(avg_own, 0.5))

                # Kelly-inspired score: edge-to-variance ratio × ownership leverage
                lu['kelly_score'] = (edge ** 2 / max(variance, 1.0)) * own_leverage

                # Blend Kelly with GPP score (50/50) for stability
                gpp_norm = lu['total_gpp_score'] / max(lu['total_gpp_score'], 0.001)
                lu['combined_score'] = 0.5 * lu['kelly_score'] + 0.5 * lu['total_gpp_score']

        # Select top lineups by combined Kelly + GPP score
        all_lineups.sort(key=lambda x: x.get('combined_score', x['total_gpp_score']), reverse=True)
...
def build_player_pool(date_str: str, use_mdn: bool = True,
                      use_ensemble: bool = True,
                      use_xgb_ownership: bool = False) -> pd.DataFrame:
    """
    Build integrated player pool merging DK salaries + projections + ownership.

    Args:
        date_str: Date string (YYYY-MM-DD)
        use_mdn: If True, use model projections; else use DK avg FPTS
        use_ensemble: If True, blend MDN v3 + Transformer v1 (MAE 4.066);
                      else use MDN v3 only (MAE 4.091)
    ...
    # 2. Load projections (ensemble or MDN-only)
    if use_mdn:
        if use_ensemble:
            proj = load_ensemble_projections(date_str)
            proj_name = "Ensemble (MDN+Trans)"
        else:
            proj = load_mdn_backtest_results(date_str)
            proj_name = "MDN v3"

        if not proj.empty:
            print(f"  {proj_name} projections: {len(proj)} players")

            dk['_match_key'] = dk['name'].apply(normalize_name_for_key)
            proj['_match_key'] = proj['player_name'].apply(normalize_name_for_key)

            # Aggregate predictions per match key (in case of duplicates)
            proj_cols = [c for c in ['predicted_fpts', 'std_fpts', 'floor_fpts', 'ceiling_fpts',
                       'p_above_10', 'p_above_15', 'p_above_20', 'p_above_25']
                       if c in proj.columns]
            proj_agg = proj.groupby('_match_key')[proj_cols].mean().reset_index()

            dk = dk.merge(proj_agg, on='_match_key', how='left')

            # Use projections where available, fallback to DK avg
            ...
            print(f"  {proj_name} match rate: {match_rate:.1f}%")
        else:
            print(f"  No projections available, using DK avg FPTS")
    ...
    xgb_ownership_used = False
    if use_xgb_ownership:
        # Try XGBoost ownership model first (production-ready, no lookahead)
        try:
            from ownership_xgb import predict_ownership_for_pool, load_ownership_model
            model_data = load_ownership_model()
            if model_data is not None:
                xgb_preds = predict_ownership_for_pool(dk, model_data)
                if xgb_preds is not None:
                    dk['predicted_ownership'] = xgb_preds.values
                    dk['predicted_ownership'] = dk['predicted_ownership'].clip(0.5, 40.0)
                    xgb_ownership_used = True
                    print(f"  XGBoost ownership model: {(xgb_preds > 0).sum()}/{len(dk)} predicted "
                          f"(mean={xgb_preds.mean():.1f}%)")
        except Exception as e:
            print(f"  XGBoost ownership model error: {e}")

    if not xgb_ownership_used:
    ...
    # 4. Apply goalie model (GBM + MoneyPuck + FC blend)
    dk = apply_goalie_model(dk)

    # 4b. Apply conformal calibration to std_fpts
    try:
        from conformal_calibration import calibrate_std_fpts, load_calibration_factors
        cal_factors = load_calibration_factors()
        if cal_factors.get('overall_factor', 1.0) != 1.0:
            dk = calibrate_std_fpts(dk, cal_factors)
            print(f"  Conformal calibration applied (scale: C={cal_factors['factors'].get('C', 1.0):.3f}, "
                  f"W={cal_factors['factors'].get('W', 1.0):.3f}, D={cal_factors['factors'].get('D', 1.0):.3f})")
    except Exception:
        pass  # Conformal calibration is optional

    # 4b2. Projection calibration — correct systematic bias in high-end projections
    # Backtest analysis shows 15+ FPTS tier is under-projected by ~1.8 FPTS on average.
    # This piecewise correction adjusts projections toward observed actual means per tier.
    # Bias data (from transformer backtest, confirmed in ensemble):
    #   0-7 FPTS: ~-0.15 (negligible)
    #   7-10 FPTS: ~+0.05 (negligible)
    #   10-15 FPTS: ~-0.03 (negligible)
    #   15+ FPTS: ~-1.82 (significant under-projection)
    try:
        # Only adjust the high-projection tier where bias is significant
        high_proj_mask = dk['projected_fpts'] >= 15.0
        if high_proj_mask.any():
            # Scale correction by how far above 15 the projection is
            # At 15: +0.5 FPTS, at 20: +1.5 FPTS, at 25+: +2.0 FPTS (capped)
            excess = (dk.loc[high_proj_mask, 'projected_fpts'] - 15.0).clip(0)
            correction = np.clip(0.5 + excess * 0.15, 0, 2.0)
            dk.loc[high_proj_mask, 'projected_fpts'] += correction
            n_corrected = high_proj_mask.sum()
            avg_correction = correction.mean()
            print(f"  Projection calibration: {n_corrected} high-end players boosted (avg +{avg_correction:.2f} FPTS)")
    except Exception:
        pass

    # 4c. Score-state deployment adjustment
    # Players on underdogs with strong trailing performance get a small boost
    # Players on favorites with strong leading performance get a small boost
    try:
        from score_state_features import compute_score_state_features
        ss_feats = compute_score_state_features(date_str, lookback_games=15)
        if not ss_feats.empty:
            dk['_name_lower'] = dk['name'].str.lower().str.strip()
            dk = dk.merge(ss_feats[['name_lower', 'ss_trailing_xg', 'ss_leading_xg',
                                     'ss_deployment_skew', 'ss_trailing_advantage']],
                          left_on='_name_lower', right_on='name_lower', how='left')
            dk.drop(columns=['_name_lower', 'name_lower'], errors='ignore', inplace=True)

            # Game script expectation: favorites (is_favorite=1) expect to lead, underdogs expect to trail
            is_fav = dk.get('is_favorite', pd.Series(0, index=dk.index)).fillna(0)
            is_underdog = 1 - is_fav

            # Spread magnitude as confidence in game script
            spread = dk.get('spread', pd.Series(0, index=dk.index)).fillna(0).abs()
            script_confidence = np.clip(spread / 3.0, 0, 1)  # Cap at 1 for 3+ goal spread

            # Score-state edge: how much better is this player in the expected game state?
            trailing_xg = dk['ss_trailing_xg'].fillna(0)
            leading_xg = dk['ss_leading_xg'].fillna(0)

            # Positive = player benefits from expected game script
            ss_edge = (is_underdog * trailing_xg + is_fav * leading_xg) * script_confidence

            # Apply as a small multiplicative adjustment (max ±5%)
            # Scale ss_edge relative to pool median
            ss_median = ss_edge[ss_edge > 0].median() if (ss_edge > 0).any() else 0.1
            ss_adjustment = np.clip(ss_edge / (ss_median + 0.01) - 1.0, -0.05, 0.05)

            dk['projected_fpts'] = dk['projected_fpts'] * (1 + ss_adjustment)
            n_adjusted = (ss_adjustment.abs() > 0.001).sum()
            avg_boost = ss_adjustment[ss_adjustment > 0.001].mean() * 100 if (ss_adjustment > 0.001).any() else 0
            print(f"  Score-state adjustment: {n_adjusted} players adjusted (avg boost: {avg_boost:+.1f}%)")

            dk.drop(columns=['ss_trailing_xg', 'ss_leading_xg', 'ss_deployment_skew',
                              'ss_trailing_advantage'], errors='ignore', inplace=True)
    except Exception:
        pass  # Score-state adjustment is optional

    # 5. Compute leverage scores
    dk = compute_leverage_scores(dk)
    print(f"  Leverage scores computed")

    # 6. Compute value metrics
...
def apply_goalie_model(dk: pd.DataFrame) -> pd.DataFrame:
    """
    Apply trained goalie GBM model to improve goalie projections.
    Uses GBM predictions blended with dk_avg_fpts as safety net (no FC dependency).
    Falls back to dk_avg_fpts if model not available.
    """
    import pickle

    model_path = Path(__file__).parent / 'data' / 'goalie_model.pkl'
    if not model_path.exists():
        return dk

    goalies_mask = dk['norm_pos'] == 'G'
    if goalies_mask.sum() == 0:
        return dk
    ...
    try:
        ...
        # GBM v2 (no FC dependency, +xG features) - 90/10 blend optimal
        w_gbm = 0.90
        w_baseline = 0.10

        # Prepare goalie features
        goalies = dk[goalies_mask].copy()

        # Add MoneyPuck features if not already present
        import sqlite3
        conn = sqlite3.connect(DB_PATH)
        mp_g = pd.read_sql("SELECT * FROM mp_goalies WHERE season = 2025 AND situation = 'all'", conn)
        mp_tm = pd.read_sql("SELECT * FROM mp_teams WHERE season = 2025 AND situation = '5on5'", conn)
        conn.close()

        # Compute MP goalie stats
        gp = mp_g['games_played'].clip(lower=1)
        mp_g['mp_sv_pct'] = 1 - (mp_g['goals'] / (mp_g['ongoal'] + 1))
        mp_g['mp_xsv_pct'] = 1 - (mp_g['xGoals'] / (mp_g['ongoal'] + 1))
        mp_g['mp_gsaa_pg'] = (mp_g['xGoals'] - mp_g['goals']) / gp
        mp_g['mp_hd_sv_pct'] = 1 - (mp_g['highDangerGoals'] / (mp_g['highDangerShots'] + 1))
        mp_g['mp_shots_pg'] = mp_g['ongoal'] / gp
        mp_g['mp_gp'] = mp_g['games_played']
        mp_g['name_lower'] = mp_g['name'].str.lower().str.strip()

        # Opponent team features
        gp_tm = mp_tm['games_played'].clip(lower=1)
        mp_tm['mp_opp_shots_pg'] = mp_tm['shotsOnGoalFor'] / gp_tm
        mp_tm['mp_opp_xGF_pg'] = mp_tm['xGoalsFor'] / gp_tm
        mp_tm['mp_opp_hd_shots_pg'] = mp_tm['highDangerShotsFor'] / gp_tm

        goalies['name_lower'] = goalies['name'].str.lower().str.strip()
        goalies = goalies.merge(
            mp_g[['name_lower', 'mp_sv_pct', 'mp_xsv_pct', 'mp_gsaa_pg',
                  'mp_hd_sv_pct', 'mp_shots_pg', 'mp_gp']],
            on='name_lower', how='left'
        )
        goalies['opp_upper'] = goalies['opponent'].str.upper()
        goalies = goalies.merge(
            mp_tm[['team', 'mp_opp_shots_pg', 'mp_opp_xGF_pg', 'mp_opp_hd_shots_pg']].rename(
                columns={'team': 'opp_upper'}),
            on='opp_upper', how='left'
        )

        # Fill missing with medians
        for f in features:
            if f not in goalies.columns:
                goalies[f] = medians.get(f, 0)
            goalies[f] = goalies[f].fillna(medians.get(f, 0))

        # Predict
        X_g = goalies[features]
        gbm_preds = model.predict(X_g)
        baseline_preds = goalies['dk_avg_fpts'].fillna(8.0).values

        # Blend: GBM-heavy with dk_avg_fpts as safety net
        blended = w_gbm * gbm_preds + w_baseline * baseline_preds

        # Update the main dataframe
        dk.loc[goalies_mask, 'projected_fpts'] = blended
        dk.loc[goalies_mask, 'std_fpts'] = 7.5  # Conservative goalie std

        print(f"  Goalie model applied: {goalies_mask.sum()} goalies projected (90% GBM v2 + 10% DK avg)")

    except Exception as e:
        print(f"  Goalie model error: {e}, using default projections")
...
def run_backtest(start_date: str = '2026-01-15', end_date: str = '2026-02-05',
                 n_lineups: int = 10, mc_iterations: int = 500,
                 use_ensemble: bool = True,
                 use_xgb_ownership: bool = False) -> pd.DataFrame:
    """
    ...
    while current <= end:
        ...
        pool = build_player_pool(date_str, use_mdn=True, use_ensemble=use_ensemble,
                                  use_xgb_ownership=use_xgb_ownership)
...
def main():
    ...
    parser.add_argument('--no-ensemble', action='store_true',
                        help='Use MDN v3 only (no transformer blend)')
    parser.add_argument('--xgb-ownership', action='store_true',
                        help='Use XGBoost ownership model instead of actual ownership')
    ...
    if args.backtest:
        results = run_backtest(
            ...
            use_xgb_ownership=args.xgb_ownership,
        )
        ...
        pool = build_player_pool(date_str, use_ensemble=not args.no_ensemble,
                                  use_xgb_ownership=args.xgb_ownership)
