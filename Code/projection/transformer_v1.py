...
def build_feature_matrix(df, predict_date, train_cutoff=None):
    ...
    try:
        ...
        if not ss_feats.empty:
            ...
            for c in [col for col in predict_games.columns if col.startswith('ss_')]:
        ...
        pass  # Score-state/rest features are optional
    ...
    for col in predict_games.columns:
        if col.startswith('ss_') and col not in all_features:
            all_features.append(col)

    # Rest / B2B features
    for col in predict_games.columns:
        if col.startswith('rest_') and col not in all_features:
            all_features.append(col)
